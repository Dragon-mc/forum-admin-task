<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.content" placeholder="回复内容" style="width: 200px;margin-right: 10px" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="回复时间" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.time | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="评论内容" min-width="80px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="回复者" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.user_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="被回复者" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.passive_user_name }}</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', 'time', 'comment_content', 'content', 'user_name', 'passive_user_name']
        const filterVal = ['id', 'time', 'comment_content', 'content', 'user_name', 'passive_user_name']
